...
async function handleShopifyWebhook(env: Env, request: Request, url: URL) {
  ...
  if (!integration) {
    ...
    try {
      ...
      const result = await verifyShopifyHmac(rawBody, signatureHeader, webhookSecret);
      verified = result.ok;
...
async function handleQboWebhook(env: Env, request: Request, url: URL) {
  ...
  if (!parsed.ok) {
    ...
    try {
      ...
      const result = await verifyQboSignature(rawBody, signatureHeader, verifierToken);
      verified = result.ok;
      verifyError = result.ok ? null : (result.error ?? "signature_mismatch");
    } catch (error) {
      verified = false;
...
function parseEnvironment(url: URL): { value: "sandbox" | "production"; invalid: boolean } {
