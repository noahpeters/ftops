...
export async function importMasterKey(env: Env): Promise<CryptoKey> {
  ...
  return await globalThis.crypto.subtle.importKey("raw", keyBytes, { name: "AES-GCM" }, false, [
    "encrypt",
    "decrypt",
  ]);
}

export async function encryptSecrets(env: Env, plaintextJson: string): Promise<EncryptedSecrets> {
...
export async function decryptSecrets(env: Env, keyId: string, ciphertext: string): Promise<string> {
