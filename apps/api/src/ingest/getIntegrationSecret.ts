...
export async function getShopifyWebhookSecret(env: Env, integration: IntegrationRow) {
...
export async function getQboVerifierToken(env: Env, integration: IntegrationRow) {
  const decrypted = await decryptSecrets(
    env,
    integration.secrets_key_id,
    integration.secrets_ciphertext
