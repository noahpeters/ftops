...
export async function createTestEnv(options?: { env?: Record<string, unknown> }) {
  ...
  try {
    const mf = new Miniflare({
      ...
      r2Buckets: {
        R2_TASK_FILES_BUCKET: "test-task-files",
        R2_CUSTOMER_EMAILS_BUCKET: "test-customer-emails",
      },
    ...
    const customerEmailsBucket = await mf.getR2Bucket("R2_CUSTOMER_EMAILS_BUCKET");
    await runMigrations(db);

    const env = {
      DB: db,
      R2_TASK_FILES_BUCKET: taskFilesBucket,
      R2_CUSTOMER_EMAILS_BUCKET: customerEmailsBucket,
...
async function execSql(db: D1Database, sql: string) {
  ...
  const statements = (cleaned.match(/\s*CREATE\s+TRIGGER\b[\s\S]*?END\s*;|[^;]+;/gi) ?? [])
