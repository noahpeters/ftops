...
describe("ingest multi-tenant routing", () => {
  it("routes shopify webhook by shop domain and verifies signature", async () => {
    ...
    const row = await db
      .prepare("SELECT workspace_id, signature_verified FROM ingest_requests WHERE provider = ?")
      .bind("shopify")
  ...
  it("stores shopify webhook with unknown account", async () => {
    ...
    const row = await db
      .prepare("SELECT verify_error, signature_verified FROM ingest_requests WHERE provider = ?")
      .bind("shopify")
